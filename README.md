...
`integration/preflight_check.py` runs the whole 29-suite contract and `selfaudit/` proves the cores
...
All twenty-four refuse to run without `PYTHONHASHSEED=0`. Test suites total **315 unit tests across 29 suites**
...
29-suite count. Five are in-repo as worked examples, each with its own tests and its own honest bound:
...
harness) and prints one green/red verdict; the per-workbench preflight below is the first of those two gates. It runs the 29 suites **and** the parity
...
by hand to assert state (asserting "29/29 green" without running it is exactly the integrity-theater this
project refuses):

```
[FOUNDRY VERIFIED]
  - 29/29 suites green; primitive parity holds (parity_proof.py)
...
- Preflight result:   [FOUNDRY VERIFIED]  (29/29 suites + PARITY HOLDS)   # paste the real tail, or BLOCKED
