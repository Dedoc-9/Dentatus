...
`integration/preflight_check.py` runs the whole 31-suite contract and `selfaudit/` proves the cores
...
All twenty-four refuse to run without `PYTHONHASHSEED=0`. Test suites total **330 unit tests across 31 suites**
...
31-suite count. Five are in-repo as worked examples, each with its own tests and its own honest bound:
...
harness) and prints one green/red verdict; the per-workbench preflight below is the first of those two gates. It runs the 31 suites **and** the parity
...
by hand to assert state (asserting "31/31 green" without running it is exactly the integrity-theater this
project refuses):

```
[FOUNDRY VERIFIED]
  - 31/31 suites green; primitive parity holds (parity_proof.py)
...
- Preflight result:   [FOUNDRY VERIFIED]  (31/31 suites + PARITY HOLDS)   # paste the real tail, or BLOCKED
