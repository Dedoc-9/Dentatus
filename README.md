...
| EXP-509 | Zeeman/Bethe Citadel; E*=β_Z, ρ(E*)=exp(2√(aE*)), a=a₀·d_stalk; excitation friction; observable + opt-in gate | `58f3f306a8f48cbe` | **open** |
| EXP-506 | Stitched local Fiedler; Galerkin coarse + partition-of-unity; dynamic spectral-diameter halo; galactic-scale fault | `d69b59b3ad635e16` | **open** |
| EXP-507 | Streaming world-sections; bounded resident set; per-section local Fiedler on demand; content-addressed eviction | `2889884aa163e496` | **open** |
...
| EXP-604 | MCL Observability dashboard; content-addressed telemetry (Fiedler fault, firewall gauge, stress curve); WebGL; cache-by-H_state | `f1f9c1a4438c3c23` | **open** |
...
| EXP-505 | Moving Claims — `track_correspondence_505` motion-compensated world frame; localized memory + healing survive motion; P_yz-invariant H_dag | landed ✓ |
...
| EXP-604 MCL (`game/observability/`) | Live WebGL dashboard — Fiedler fault, firewall gauge, agency stress curve; content-addressed telemetry (Fiedler decomposition cached per unique H_state); SSE push stub | landed ✓ |
| EXP-605 live stream (`sse_server.py`) | Live reality conduit — keyframe+delta coarse codec + content-addressed sections over SSE; bitrate ∝ rate-of-change; sections pulse in with no pop-in | landed ✓ |
...
EXP-603 target: the agency loop — LLM iterate-to-stable against permanent verified addresses, with an agency latch bounding outer-loop backreaction.
