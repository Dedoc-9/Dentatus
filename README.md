...
`integration/preflight_check.py` runs the whole 32-suite contract and `selfaudit/` proves the cores
...
Twenty-five components group into a few families, each a different hat on the one idea. **The audit core**
...
All twenty-five refuse to run without `PYTHONHASHSEED=0`. Test suites total **353 unit tests across 32 suites**
...
32-suite count. Five are in-repo as worked examples, each with its own tests and its own honest bound:
...
harness) and prints one green/red verdict; the per-workbench preflight below is the first of those two gates. It runs the 32 suites **and** the parity
...
by hand to assert state (asserting "32/32 green" without running it is exactly the integrity-theater this
project refuses):

```
[FOUNDRY VERIFIED]
  - 32/32 suites green; primitive parity holds (parity_proof.py)
...
- Preflight result:   [FOUNDRY VERIFIED]  (32/32 suites + PARITY HOLDS)   # paste the real tail, or BLOCKED
