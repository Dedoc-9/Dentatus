...
PYTHONHASHSEED=0 python3 conservation.py             # the THESIS test: one field vs specialists (Outcome C = coordination layer)
PYTHONHASHSEED=0 python3 tests/test_causal_runtime.py  # 71 unit tests
```
...
| `conservation.py` | the **thesis test** — one shared field vs domain specialists; **Outcome C**: no within-domain magic, conserved advantage is cross-domain coordination (the budget split), falsifiable (drift loses to equal-split floor) |
| `tests/test_causal_runtime.py` | 71 unit tests (incl. the cardinal invariant under dini, ghost rectification, blind discovery, the epistemic-trap locks) |
