...
**Current verification state: 217 tests passing across 20 suites (+ coupled/uncoupled parity proof).**
...
[ Workbench flow ]    Generate -> preflight (20 suites) -> replay verification -> fast deploy
```
...
Cores are frozen; the sixteen siblings import them read-only (the Sibling Law, S4). `selfaudit/` keeps the
two cores honest against a pinned baseline; `integration/preflight_check.py` runs all twenty suites + the
