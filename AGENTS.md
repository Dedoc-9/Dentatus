...
is bound by the contract below. The boundaries are enforced by 304 tests across 28
...
**Current verification state: 304 tests passing across 28 suites (+ coupled/uncoupled parity proof).**
...
1. `integration/preflight_check.py` prints `[FOUNDRY VERIFIED]` (it actually ran the 28 suites + parity).
...
1. **One replayable gate instead of a CI matrix (`integration/preflight_check.py` + `parity_proof.py`).**
   The preflight *is* the definition of done: it runs all 28 suites + the coupled/uncoupled parity proof as
...
[ Workbench flow ]    Generate -> preflight (28 suites) -> replay verification -> fast deploy
```
