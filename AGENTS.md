...
is bound by the contract below. The boundaries are enforced by 330 tests across 31
...
**Current verification state: 330 tests passing across 31 suites (+ coupled/uncoupled parity proof).**
...
1. `integration/preflight_check.py` prints `[FOUNDRY VERIFIED]` (it actually ran the 31 suites + parity).
...
1. **One replayable gate instead of a CI matrix (`integration/preflight_check.py` + `parity_proof.py`).**
   The preflight *is* the definition of done: it runs all 31 suites + the coupled/uncoupled parity proof as
...
[ Workbench flow ]    Generate -> preflight (31 suites) -> replay verification -> fast deploy
```
