...
is bound by the contract below. The boundaries are enforced by 326 tests across 30
...
**Current verification state: 326 tests passing across 30 suites (+ coupled/uncoupled parity proof).**
...
1. `integration/preflight_check.py` prints `[FOUNDRY VERIFIED]` (it actually ran the 30 suites + parity).
...
1. **One replayable gate instead of a CI matrix (`integration/preflight_check.py` + `parity_proof.py`).**
   The preflight *is* the definition of done: it runs all 30 suites + the coupled/uncoupled parity proof as
...
[ Workbench flow ]    Generate -> preflight (30 suites) -> replay verification -> fast deploy
```
