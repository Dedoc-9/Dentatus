...
is bound by the contract below. The boundaries are enforced by 294 tests across 27
...
**Current verification state: 294 tests passing across 27 suites (+ coupled/uncoupled parity proof).**
...
1. `integration/preflight_check.py` prints `[FOUNDRY VERIFIED]` (it actually ran the 27 suites + parity).
...
1. **One replayable gate instead of a CI matrix (`integration/preflight_check.py` + `parity_proof.py`).**
   The preflight *is* the definition of done: it runs all 27 suites + the coupled/uncoupled parity proof as
...
[ Workbench flow ]    Generate -> preflight (27 suites) -> replay verification -> fast deploy
```
