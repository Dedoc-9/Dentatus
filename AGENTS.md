...
is bound by the contract below. The boundaries are enforced by 315 tests across 29
...
**Current verification state: 315 tests passing across 29 suites (+ coupled/uncoupled parity proof).**
...
1. `integration/preflight_check.py` prints `[FOUNDRY VERIFIED]` (it actually ran the 29 suites + parity).
...
1. **One replayable gate instead of a CI matrix (`integration/preflight_check.py` + `parity_proof.py`).**
   The preflight *is* the definition of done: it runs all 29 suites + the coupled/uncoupled parity proof as
...
[ Workbench flow ]    Generate -> preflight (29 suites) -> replay verification -> fast deploy
```
