...
**315 unit tests across 29 suites**, all gated by one preflight). The siblings are deliberately decoupled —
...
| Property | Evidence |
|---|---|
| The whole workbench passes one gate — 29/29 suites + coupled/uncoupled parity, run as real subprocesses | `integration/preflight_check.py` → `[FOUNDRY VERIFIED]` |
