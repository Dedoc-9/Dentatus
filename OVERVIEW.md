...
**304 unit tests across 28 suites**, all gated by one preflight). The siblings are deliberately decoupled —
...
| Property | Evidence |
|---|---|
| The whole workbench passes one gate — 28/28 suites + coupled/uncoupled parity, run as real subprocesses | `integration/preflight_check.py` → `[FOUNDRY VERIFIED]` |
