"""
run_p_invariance_exp409.py — Fork B: EXP-409 P_yz Invariance

Protocol: exp409-v1
Declaration hash: 32a0fbef5f6e5e1eb0e33adc0ee3e25b332e3586b30e0497f81194fb48b799f6

phi_fb_hysteresis_ema reads only ||S_A||, ||Z_A||, ||S_D|| (norms), scalar scene_n,
scalar bze_ema_prev, bool maint_latched. All norm inputs are P_yz-invariant.
maint_latched is a primary scalar: same value for fwd and mir given identical cold start.
Both trajectories latch at the same step (P_yz-symmetric attractor boundary).

Tests [1-5]:
  [1]  declaration_hash
  [2]  bze_fwd(n) == bze_mir(n) for all n
...
def make_mu_stalk(stalk, S_A_init=None, S_C_init=None, S_D_init=None, S_init=None):
    prov  = Provenance(parent_ids=(), operator_id="seed_exp409_pyz", timestamp=now_iso())
    claim = Claim(provenance=prov, payload="scene:exp409:pyz",
...
BD_f_traj,  BD_m_traj  = [], []
lc_f_traj,  lc_m_traj  = [], []
latch_f_traj, latch_m_traj = [], []

mu_fwd = make_mu_stalk(stalk_fwd)
mu_mir = make_mu_stalk(stalk_mir)
bze_ema_f = float(_BETA_Z_313); maint_f = False
bze_ema_m = float(_BETA_Z_313); maint_m = False
...
for n in range(N_STEPS):
    ...
    (mu_fwd, _, _, bze_f, raw_f, BA_f, BD_f, _, _,
     aeff_f, maint_f) = apply_gamma_409_recursive(
        mu=mu_fwd, claim_id=cid_f, scene_n=n,
        bze_ema_prev=bze_ema_f, maint_latched=maint_f, **SHARED)
    (mu_mir, _, _, bze_m, raw_m, BA_m, BD_m, _, _,
     aeff_m, maint_m) = apply_gamma_409_recursive(
        mu=mu_mir, claim_id=cid_m, scene_n=n,
        bze_ema_prev=bze_ema_m, maint_latched=maint_m, **SHARED)
    ...
    latch_f_traj.append(maint_f); latch_m_traj.append(maint_m)

print(f"\n  P_yz sequential N={N_STEPS} (phi_fb_hysteresis_ema):")
print(f"  bze_fwd: {[round(b,2) for b in bze_f_traj]}")
print(f"  bze_mir: {[round(b,2) for b in bze_m_traj]}")
print(f"  latch_f: {[int(l) for l in latch_f_traj]}")
print(f"  latch_m: {[int(l) for l in latch_m_traj]}")
...
print(f"[2] PASS  max|bze_fwd-bze_mir|={max_bze:.2e} (phi_fb_hysteresis_ema P_yz-invariant)")
...
# Latch symmetry check (informational)
latch_agree = all(latch_f_traj[n] == latch_m_traj[n] for n in range(N_STEPS))
print(f"      latch_fwd==latch_mir for all n: {latch_agree} (P_yz-symmetric latch)")

print(f"\n=== EXP-409 Fork B: 5/5 PASS ===")
print(f"    max|bze_fwd-bze_mir|={max_bze:.2e}")
print(f"    max|B_A_fwd-B_A_mir|={max_BA:.2e}")
print(f"    max|B_D_fwd-B_D_mir|={max_BD:.2e}")
print(f"    alpha_disc={_ALPHA_DISC_409}  alpha_maint={_ALPHA_MAINT_409}  threshold={_BETA_THRESHOLD_409}")
