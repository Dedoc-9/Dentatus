...
### Scaling/Design Backlog · Non-orientable boundary + fractal substrate (MEASURED · UN-LICENSED)
- **Status:** design compass only. NOT sealed, NOT wired (profiler EXP-532 shows 7x headroom; the duel needs neither wraparound nor fractal maps). `forge/menger_klein_proof.py`.
- **Corrections of record (an earlier draft was wrong on both):**
  - *Fractal porosity:* the draft claimed 40.62%; its own code on a 64-grid actually yields 28.15%, and 64 != 3^k so it isn't even a faithful Sierpinski carpet. On a faithful 81=3^4 grid the level-4 solid count is EXACTLY 8^4=4096, porosity = analytic 1-(8/9)^4 = **37.57%**.
  - *Klein boundary:* the draft flipped y on every wrap event; the non-orientable horizontal identification (x,y)~(x+GX, GY-1-y) is an INVOLUTION, so the flip must follow lap PARITY (1 lap flips, 2 laps identity). Draft `wrap(128,10)` wrongly returned a flipped value — broken for teleports / fast projectiles. Fixed + invariant-checked.
- **Topological verdict (rigor, not hype):**
