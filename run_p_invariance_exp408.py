"""
run_p_invariance_exp408.py — Fork B: EXP-408 P_yz Invariance

Protocol: exp408-v1
Declaration hash: d1945e6736a284a33cf648ac0d7d06491e0bea704bd6fa69ca7beac277562055

phi_fb_ascending_ema reads only ||S_A||, ||Z_A||, ||S_D|| (norms), scalar scene_n,
scalar bze_ema_prev. All inputs are P_yz-invariant -> phi_fb_ascending_ema is
P_yz-invariant by construction. alpha_eff and bze_ema_prev are primary scalars:
same values for fwd and mir given identical cold start.
...
from engine.operators import (
    apply_gamma_408_recursive,
    _GAMMA_INF_A_408, _GAMMA_INF_D_408, _TAU_WARMUP_408,
    _ALPHA_MIN_408, _ALPHA_MAX_408, _TAU_ALPHA_408,
    _BETA_Z_MIN_408, _BETA_Z_313,
)
...
def make_mu_stalk(stalk, S_A_init=None, S_C_init=None, S_D_init=None, S_init=None):
    prov  = Provenance(parent_ids=(), operator_id="seed_exp408_pyz", timestamp=now_iso())
    claim = Claim(provenance=prov, payload="scene:exp408:pyz",
...
bze_ema_f = float(_BETA_Z_313)
bze_ema_m = float(_BETA_Z_313)
...
for n in range(N_STEPS):
    ...
    mu_fwd, _, _, bze_f, raw_f, BA_f, BD_f, _, _, aeff_f = apply_gamma_408_recursive(
        mu=mu_fwd, claim_id=cid_f, scene_n=n, bze_ema_prev=bze_ema_f, **SHARED)
    mu_mir, _, _, bze_m, raw_m, BA_m, BD_m, _, _, aeff_m = apply_gamma_408_recursive(
...
print(f"\n  P_yz sequential N={N_STEPS} (phi_fb_ascending_ema):")
...
print(f"[2] PASS  max|bze_fwd-bze_mir|={max_bze:.2e} (phi_fb_ascending_ema P_yz-invariant)")
