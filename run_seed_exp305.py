"""
run_seed_exp305.py -- EXP-305 Axial Restoration Engine seed test.

Protocol: exp305-v1
declaration_hash: 2a05be65524a8ce244ea9597917a5a3f771094a2bc25fd33f801ad1c6cc99e64

Test sequence:
  1. Seed_0_axial:  d=11, stalk=[1,1,1,1, 0.5,0.5,0.5,1, 0,0,1], bbox=[0,1]^3
  2. Gamma305(Seed, axis_bisect_x) -> 2 children
       - Sector A conservation:  ||A_x0 + A_x1 - A_seed|| < 1e-10
       - Sector B centroid:      child.stalk[4:7] = centroid(child.bbox)
       - Sector B w=1 invariant: child.stalk[7] == 1.0
       - Sector B barycentric:   0.5*stalk_B(x0) + 0.5*stalk_B(x1) ~ stalk_B(seed)
       - Sector C unit-norm:     ||child.stalk[8:11]||_2 == 1.0
       - Block-diagonal F:       all off-diagonal sector cross blocks < 1e-7
  3. Gamma305(Seed, octree_split) -> 8 children (all invariants above, N=8)
  4. is_unit_norm violation -> mutate child normal -> is_unit_norm=False
  5. is_valid_block_diagonal violation -> inject cross-sector coupling -> False
  6. Psi + R3 on axis_bisect_x children
"""

import sys
import json
import hashlib
import copy
import numpy as np
sys.path.insert(0, ".")

from engine.state import (
    MuState, Claim, Entailment, EntailmentType, Provenance,
    PROTOCOL_VERSION, now_iso, ALPHA_DEFAULT as ALPHA
)
from engine.operators import (
    apply_gamma_305, apply_psi, apply_omega,
    tensor_product_artifact, check_r3,
    SECTOR_A_DIMS, SECTOR_B_DIMS, SECTOR_C_DIMS
)
from engine.validity import (
    is_valid, is_valid_b, is_spatially_valid,
    is_unit_norm, is_valid_block_diagonal
)

# ---------------------------------------------------------------------------
# Load seed declaration and verify hash
# ---------------------------------------------------------------------------
with open("studies/exp305_axial_engine/SEED_DECLARATION_exp305.json") as f:
    DECL = json.load(f)

stored_hash = DECL["declaration_hash"]
verify_fields = {k: v for k, v in DECL.items() if k not in ("declaration_hash", "status")}
canonical = json.dumps(verify_fields, sort_keys=True, separators=(',', ':'))
computed_hash = hashlib.sha256(canonical.encode()).hexdigest()
assert computed_hash == stored_hash, (
    f"SEED HASH MISMATCH\n  stored:   {stored_hash}\n  computed: {computed_hash}"
)
print(f"Seed hash verified: {stored_hash[:16]}...")

BETA = DECL.get("beta", 0.1)
B0   = DECL.get("B0", 100.0)
D    = DECL["d"]                          # 11
SEED_STALK  = np.array(DECL["seed_stalk"], dtype=float)
BBOX_LO     = np.array(DECL["seed_bbox_lo"], dtype=float)
BBOX_HI     = np.array(DECL["seed_bbox_hi"], dtype=float)

b0A, b1A = SECTOR_A_DIMS[0], SECTOR_A_DIMS[-1] + 1   # [0:4]
b0B, b1B = SECTOR_B_DIMS[0], SECTOR_B_DIMS[-1] + 1   # [4:8]
b0C, b1C = SECTOR_C_DIMS[0], SECTOR_C_DIMS[-1] + 1   # [8:11]

def fmt_A(stalk): return f"[{', '.join(f'{v:.3f}' for v in stalk[b0A:b1A])}]"
def fmt_B(stalk): return f"[{', '.join(f'{v:.3f}' for v in stalk[b0B:b1B])}]"
def fmt_C(stalk): return f"[{', '.join(f'{v:.4f}' for v in stalk[b0C:b1C])}]"
def bbox_str(bbox):
    if bbox is None: return "None"
    return f"[[{', '.join(f'{v:.2f}' for v in bbox[0])}] -> [{', '.join(f'{v:.2f}' for v in bbox[1])}]]"

# ---------------------------------------------------------------------------
# 0. Build Seed_0_axial
# ---------------------------------------------------------------------------
seed_prov = Provenance(parent_ids=(), operator_id="Seed_0_axial", timestamp=now_iso())
seed = Claim(provenance=seed_prov,
             payload="Seed_0_axial: d=11 EXP-305, mass=1 white unit cube nz=1.",
             stalk=SEED_STALK.copy(), t=0, bbox=(BBOX_LO, BBOX_HI))
mu0 = MuState(
    claims={seed.id: seed},
    entailments={},
    active=frozenset([seed.id]),
    t=0, S=np.zeros(D), alpha=ALPHA,
)
mu0.seal()

print(f"\nSeed_0_axial  H_0: {mu0._H[:16]}...")
print(f"  Sector A (mass,r,g,b): {fmt_A(seed.stalk)}")
print(f"  Sector B (x,y,z,w):    {fmt_B(seed.stalk)}")
print(f"  Sector C (nx,ny,nz):   {fmt_C(seed.stalk)}")
print(f"  ||Sector C||: {np.linalg.norm(seed.stalk[b0C:b1C]):.6f}")
print(f"  bbox: {bbox_str(seed.bbox)}")

assert seed.stalk[7] == 1.0, "seed w != 1"
assert abs(np.linalg.norm(seed.stalk[b0C:b1C]) - 1.0) < 1e-8, "seed ||nC|| != 1"
assert is_valid_b(mu0), "seed fails is_valid_b"
assert is_unit_norm(mu0), "seed fails is_unit_norm"
assert is_valid_block_diagonal(mu0), "seed fails is_valid_block_diagonal (trivially true for seed)"

# ---------------------------------------------------------------------------
# 1. Gamma305(Seed, axis_bisect_x) -> 2 children
# ---------------------------------------------------------------------------
print(f"\n--- Gamma305(Seed, axis_bisect_x) ---")
mu1, g_cost = apply_gamma_305(
    mu=mu0, claim_id=seed.id, partition_key="axis_bisect_x",
    payloads=["A_x0", "A_x1"], beta=BETA, budget=B0, spent=0.0
)
print(f"  cost={g_cost:.4f}  H_1={mu1._H[:16]}...")

# Identify children by bbox lo_x
all_ids = list(mu1.active)
cx0_id = next(cid for cid in all_ids if mu1.claims[cid].bbox[0][0] < 0.1)
cx1_id = next(cid for cid in all_ids if mu1.claims[cid].bbox[0][0] > 0.4)
cx0 = mu1.claims[cx0_id]; cx1 = mu1.claims[cx1_id]

print(f"  lower-x half:")
print(f"    Sector A: {fmt_A(cx0.stalk)}   Sector B: {fmt_B(cx0.stalk)}")
print(f"    Sector C: {fmt_C(cx0.stalk)}   ||C||={np.linalg.norm(cx0.stalk[b0C:b1C]):.6f}")
print(f"    bbox: {bbox_str(cx0.bbox)}")
print(f"  upper-x half:")
print(f"    Sector A: {fmt_A(cx1.stalk)}   Sector B: {fmt_B(cx1.stalk)}")
print(f"    Sector C: {fmt_C(cx1.stalk)}   ||C||={np.linalg.norm(cx1.stalk[b0C:b1C]):.6f}")
print(f"    bbox: {bbox_str(cx1.bbox)}")

# Sector A conservation
seed_A = seed.stalk[b0A:b1A]
cons_A = float(np.linalg.norm((cx0.stalk[b0A:b1A] + cx1.stalk[b0A:b1A]) - seed_A))
print(f"  Sector A conservation ||A_x0+A_x1-A_seed||: {cons_A:.2e}")
assert cons_A < 1e-10

# Sector B centroid check
for cid, lbl in [(cx0_id, "lower"), (cx1_id, "upper")]:
    c = mu1.claims[cid]
    c_lo, c_hi = c.bbox
    expected_c = (c_lo + c_hi) / 2.0
    actual_c = c.stalk[b0B:b0B+3]
    err = float(np.linalg.norm(actual_c - expected_c))
    print(f"  Sector B centroid ({lbl}): expected {expected_c}, err={err:.2e}")
    assert err < 1e-10, f"centroid mismatch: {actual_c} vs {expected_c}"

# w=1 invariant
assert abs(cx0.stalk[7] - 1.0) < 1e-10, f"cx0 w={cx0.stalk[7]}"
assert abs(cx1.stalk[7] - 1.0) < 1e-10, f"cx1 w={cx1.stalk[7]}"
print(f"  Sector B w=1 invariant: PASS")

# Sector B barycentric
omega0 = mu1.entailments[(seed.id, cx0_id)].omega
omega1 = mu1.entailments[(seed.id, cx1_id)].omega
bary_err = float(np.linalg.norm(
    omega0 * cx0.stalk[b0B:b1B] + omega1 * cx1.stalk[b0B:b1B] - seed.stalk[b0B:b1B]
))
print(f"  Sector B barycentric err: {bary_err:.2e}")
assert bary_err < 1e-10

# Sector C unit-norm check
for cid, lbl in [(cx0_id, "lower"), (cx1_id, "upper")]:
    n_norm = float(np.linalg.norm(mu1.claims[cid].stalk[b0C:b1C]))
    print(f"  Sector C ||n|| ({lbl}): {n_norm:.8f}")
    assert abs(n_norm - 1.0) < 1e-8, f"unit-norm violated: {n_norm}"

# Block-diagonal restriction map check
for cid, lbl in [(cx0_id, "lower"), (cx1_id, "upper")]:
    F = mu1.entailments[(seed.id, cid)].restriction
    off_diag = max(
        np.linalg.norm(F[0:4, 4:11]),
        np.linalg.norm(F[4:8, 0:4]),
        np.linalg.norm(F[4:8, 8:11]),
        np.linalg.norm(F[8:11, 0:8]),
    )
    print(f"  Block-diagonal off-diag max ({lbl}): {off_diag:.2e}")
    assert off_diag < 1e-7, f"sector coupling detected: {off_diag}"

# Combined validity
assert is_valid(mu1),               "FAIL: is_valid (Sector A)"
assert is_valid_b(mu1),             "FAIL: is_valid_b (Sector B)"
assert is_unit_norm(mu1),           "FAIL: is_unit_norm (Sector C)"
assert is_spatially_valid(mu1),     "FAIL: is_spatially_valid"
assert is_valid_block_diagonal(mu1),"FAIL: is_valid_block_diagonal"
print(f"  All 5 validity predicates: PASS")

# ---------------------------------------------------------------------------
# 2. Gamma305(Seed, octree_split) -> 8 children
# ---------------------------------------------------------------------------
print(f"\n--- Gamma305(Seed, octree_split) -> 8 octants ---")
oct_payloads = [f"oct{i}" for i in range(8)]
mu_oct, oct_cost = apply_gamma_305(
    mu=mu0, claim_id=seed.id, partition_key="octree_split",
    payloads=oct_payloads, beta=BETA, budget=B0, spent=0.0
)
print(f"  cost={oct_cost:.4f}  H={mu_oct._H[:16]}...")
assert len(mu_oct.active) == 8

stalk_A_sum = np.zeros(4)
omega_sum = 0.0
for oid in mu_oct.active:
    child = mu_oct.claims[oid]
    stalk_A_sum += child.stalk[b0A:b1A]
    # centroid
    c_lo, c_hi = child.bbox
    expected_c = (c_lo + c_hi) / 2.0
    actual_c = child.stalk[b0B:b0B+3]
    assert np.linalg.norm(actual_c - expected_c) < 1e-10, \
        f"Octant centroid wrong: {actual_c} vs {expected_c}"
    # w=1
    assert abs(child.stalk[7] - 1.0) < 1e-10, f"Octant w={child.stalk[7]}"
    # unit-norm Sector C
    n_norm = float(np.linalg.norm(child.stalk[b0C:b1C]))
    assert abs(n_norm - 1.0) < 1e-8, f"Octant Sector C ||n||={n_norm}"
    ent = mu_oct.entailments[(seed.id, oid)]
    omega_sum += ent.omega

cons_A_oct = float(np.linalg.norm(stalk_A_sum - seed.stalk[b0A:b1A]))
print(f"  Sector A conservation (N=8): {cons_A_oct:.2e}")
assert cons_A_oct < 1e-10

assert abs(omega_sum - 1.0) < 1e-10, f"omega sum = {omega_sum}"
print(f"  omega sum = {omega_sum:.10f}")
assert is_valid_b(mu_oct),             "FAIL: is_valid_b octree"
assert is_unit_norm(mu_oct),           "FAIL: is_unit_norm octree"
assert is_spatially_valid(mu_oct),     "FAIL: is_spatially_valid octree"
assert is_valid_block_diagonal(mu_oct),"FAIL: is_valid_block_diagonal octree"
print(f"  Sector A conservation (N=8):          PASS")
print(f"  Sector B centroid + w=1 (N=8):        PASS")
print(f"  Sector C unit-norm (N=8):             PASS")
print(f"  All 5 validity predicates (N=8):      PASS")

for oid in sorted(mu_oct.active):
    c = mu_oct.claims[oid]
    ent = mu_oct.entailments[(seed.id, oid)]
    n_norm = float(np.linalg.norm(c.stalk[b0C:b1C]))
    print(f"    det_sign={ent.det_sign:+d}  ||n||={n_norm:.6f}  bbox={bbox_str(c.bbox)}")

# ---------------------------------------------------------------------------
# 3. is_unit_norm violation: mutate Sector C normal
# ---------------------------------------------------------------------------
print(f"\n--- is_unit_norm violation: set n=[2,0,0] ---")
mu_bad_norm = copy.deepcopy(mu1)
bad_id = cx0_id
bad_claim = mu_bad_norm.claims[bad_id]
bad_stalk = bad_claim.stalk.copy()
bad_stalk[b0C:b1C] = np.array([2.0, 0.0, 0.0])   # ||n|| = 2.0, not 1.0
object.__setattr__(bad_claim, 'stalk', bad_stalk)
mu_bad_norm.claims[bad_id] = bad_claim
result = is_unit_norm(mu_bad_norm)
print(f"  is_unit_norm with ||n||=2.0: {result}  {'PASS' if not result else 'FAIL'}")
assert not result

# ---------------------------------------------------------------------------
# 4. is_valid_block_diagonal violation: inject cross-sector coupling
# ---------------------------------------------------------------------------
print(f"\n--- is_valid_block_diagonal violation: inject F[0,4] = 0.5 ---")
mu_bad_bd = copy.deepcopy(mu1)
ent_key = (seed.id, cx0_id)
bad_ent = mu_bad_bd.entailments[ent_key]
bad_F = bad_ent.restriction.copy()
bad_F[0, 4] = 0.5   # inject A->B cross-sector coupling
object.__setattr__(bad_ent, 'restriction', bad_F)
mu_bad_bd.entailments[ent_key] = bad_ent
result_bd = is_valid_block_diagonal(mu_bad_bd)
print(f"  is_valid_block_diagonal with F[0,4]=0.5: {result_bd}  {'PASS' if not result_bd else 'FAIL'}")
assert not result_bd

# ---------------------------------------------------------------------------
# 5. Psi(cx0, cx1) + R3
# ---------------------------------------------------------------------------
print(f"\n--- Psi(cx0, cx1) -> v_blend ---")
mu2, psi_cost = apply_psi(
    mu=mu1, claim_ids=[cx0_id, cx1_id],
    synthesis_key="axial_blend",
    payload="blend_x0_x1_exp305",
    weights=[0.5, 0.5],
    beta=BETA, budget=B0, spent=g_cost
)
spent = g_cost + psi_cost
mu2.seal()
blend_id = next(iter(mu2.active))
blend = mu2.claims[blend_id]
print(f"  cost={psi_cost:.4f}  blend Sector A: {fmt_A(blend.stalk)}")
print(f"  blend Sector B: {fmt_B(blend.stalk)}  Sector C: {fmt_C(blend.stalk)}")
print(f"  blend ||Sector C||: {np.linalg.norm(blend.stalk[b0C:b1C]):.6f}")
# Note: blend ||n|| will not be 1.0 in general (weighted avg of two unit vectors)
# This is expected and documented: Psi does not enforce is_unit_norm.

# R3 check
arts_mu1 = []
for cid in [cx0_id, cx1_id]:
    art = apply_omega(mu=mu1, claim_id=cid, confluence_cert=f"cert_{cid[:8]}")
    arts_mu1.append(art)

Art_blend = apply_omega(mu=mu2, claim_id=blend_id, confluence_cert="cert_blend")
Art_A = arts_mu1[0]; Art_B = arts_mu1[1]
Art_tensor = tensor_product_artifact(Art_A, Art_B, weights=[0.5, 0.5])
r3_pass, r3_err = check_r3(Art_blend, Art_tensor)
K_gain = Art_tensor["K_bound_sum"] - Art_blend["K_bound"]
print(f"  R3 stalk err: {r3_err:.2e}  K_gain={K_gain} bytes")
assert r3_pass, f"R3 FAIL: err={r3_err}"

# ---------------------------------------------------------------------------
# Summary
# ---------------------------------------------------------------------------
print(f"\n=== EXP-305 Seed PASS ===")
print(f"  declaration_hash: {stored_hash}")
print(f"  H_0:              {mu0._H[:16]}...")
print(f"  H_1 (bisect_x):   {mu1._H[:16]}...")
print(f"  H_oct (octree):   {mu_oct._H[:16]}...")
print(f"  H_2 (post-Psi):   {mu2._H[:16]}...")
print(f"  d={D}  schema: {DECL['stalk_schema']['dims']}")
print(f"  Sector A [0-3]  sum conservation:          PASS")
print(f"  Sector B [4-7]  barycentric + w=1:         PASS")
print(f"  Sector C [8-10] unit-norm (Phi_C):         PASS")
print(f"  Block-diagonal F (no sector coupling):     PASS")
print(f"  is_unit_norm violation detected:           PASS")
print(f"  is_valid_block_diagonal violation detected: PASS")
print(f"  Gamma305 octree (N=8):                     PASS")
print(f"  R3 err: {r3_err:.2e}")
print(f"  Budget spent: {spent:.4f} / {B0}")
